...
Imager.prototype = {
  ...
  upload: function (files, callback, variant, directory) {
    var self = this;
    ...
    var prepare = function (file, fn) {
      ...
      self.prepareUpload(file, filename, variant, directory, fn);
    };
  ...
  prepareUpload: function (file, filename, variant, directory, fn) {
    ...
    if (variants.resize) {
      Object.keys(variants.resize).forEach(function (name) {
        var processFiles = function (cb) {
          ...
          self.resizeFile(file, directory, preset, filename, cb);
        };
    ...
    if (variants.crop) {
      Object.keys(variants.crop).forEach(function (name) {
        var processFiles = function (cb) {
          ...
          self.cropFile(file, directory, preset, filename, cb);
        };
        asyncArr.push(processFiles);
      });
    }
    ...
    if (variants.resizeAndCrop) {
      Object.keys(variants.resizeAndCrop).forEach(function (name) {
        var processFiles = function (cb) {
          ...
          self.resizeAndCropFile(file, directory, preset, filename, cb);
        };
        asyncArr.push(processFiles);
      });
    }
    ...
    async.parallel(asyncArr, function (err, results) {
      ...
      var f = _us.uniq(results).toString();
  ...
  resizeFile: function (file, directory, preset, fname, cb) {
    var filename = fname + contentType[file['type']];
    var self = this;
    filename = preset.name + '_' + filename;
    var remoteFile = directory + '/' + filename;
    var dest = tempDir + filename;
  ...
  cropFile: function (file, directory, preset, fname, cb) {
    var filename = fname + contentType[file['type']];
    var self = this;
    filename = preset.name + '_' + filename;
    var remoteFile = directory + '/' + filename;
  ...
  resizeAndCropFile: function (file, directory, preset, fname, cb) {
    var filename = fname + contentType[file['type']];
    var self = this;
    filename = preset.name + '_' + filename
    var remoteFile = directory + '/' + filename;
    ...
    if (variants.resize) {
      Object.keys(variants.resize).forEach(function (preset) {
        var removeFiles = function (cb) {
          self['removeFrom' + self.storage](file, directory, preset, cb);
        };
  ...
  removeFromRackspace: function (file, directory, preset, cb) {
    var self = this;
    var client = cloudfiles.createClient(this.config['storage'][this.storage]);
    var rsContainer = this.config['storage'][this.storage].container;
  ...
  removeFromS3: function (file, directory, preset, cb) {
    var self = this;
    var client = knox.createClient(this.config['storage'][this.storage]);
    var remoteFile = directory + '/' + preset + '_' + file;
